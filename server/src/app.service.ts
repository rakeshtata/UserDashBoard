...
export class AuthenicatorService {
  getAuthentication(args: { name: string }): Promise<any> {
    return axios
      .get(`http://jsonServer-app:8000/auth/${args.name}`, {
...
export class ActivityService {
  getActivities(args: { id: string }): Promise<any> {
    ...
    return axios
      .get(`http://jsonServer-app:8000/data/${id}`, {
        headers: { connection: 'keep-alive' },
      })
...
export class UserService {
  ...
  getUser(args: { id: string }): Promise<any> {
    return axios
      .get(`http://jsonServer-app:8000/data/${args.id}`, {
        headers: { connection: 'keep-alive' },
      })
      .then((resp) => resp.data);
  }
  ...
  getUsers(): Promise<any> {
    // return axios.get(`http://jsonServer-app:8000/data`, { headers: { connection: "keep-alive" } })
    //             .then(resp => resp.data);
  ...
  async addUser({ name, gender, age }): Promise<any> {
    return axios
      .post('http://jsonServer-app:8000/data', { name, gender, age })
      .then((res) => res.data);
  ...
  editUser({ name, gender, age, id }): Promise<any> {
    return axios
      .patch('http://jsonServer-app:8000/data', { name, gender, age, id })
      .then((res) => res.data);
  ...
  deleteUser(args: { id: string }): Promise<any> {
    return axios
      .delete(`http://jsonServer-app:8000/data/${args.id}`, {
        headers: { connection: 'keep-alive' },
      })
      .then((resp) => resp.data);
